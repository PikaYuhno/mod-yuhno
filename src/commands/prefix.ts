...
export default class Ping {
    ...
    public async run() {
        ...
        if (this._args.length === 0) return this._message.reply(`${Constants.PREFIX_SUCCESS} Your current Prefix is: ${this._client['guildConfig'].get(guildId).prefix}`);
        ...
        this._message.reply(`${Constants.PREFIX_SUCCESS} Prefix updated to: ${newConfig.prefix}`);
    }
}