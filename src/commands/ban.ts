...
export default class Ban {
    ...
    public async run() {
        if(this._args.length === 0) {
            return this._message.channel.send(`${Constants.PREFIX_FAILURE} Please provide the required paramenters.`);
        }
        const user = this._message.mentions.users.first() || this._client.users.cache.get(this._args[0]);
        this._args.shift();
        const reason = this._args.join(" ") || "No reason provided";

        if(!user) return this._message.channel.send(`${Constants.PREFIX_FAILURE} User was not found.`);
        try {
            await this._message.guild.members.ban(user, {reason});
        } catch (error) {
            return this._message.channel.send(`${Constants.PREFIX_FAILURE} Something went wrong...`);
        }
        this._message.channel.send(`${Constants.PREFIX_SUCCESS} ${user.tag} was successfully banned!`);
    }
